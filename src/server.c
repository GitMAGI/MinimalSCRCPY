...
enable_tunnel(struct server *server) {
    ...
    warningLog("'adb reverse' failed, fallback to 'adb forward'");
...
connect_to_server(uint16_t port, uint32_t attempts, uint32_t delay) {
    do {
        debugLog("Remaining connection attempts: %d", (int) attempts);
...
close_socket(socket_t *socket) {
    ...
    if (!net_close(*socket)) {
        warningLog("Cannot close socket");
        return;
    }
...
server_start(struct server *server, const char *serial,
    ...
    if (!server->tunnel_forward) {
        ...
        if (server->server_socket == INVALID_SOCKET) {
            errorLog("Could not listen on port %" PRIu16, local_port);
...
server_stop(struct server *server) {
    if (!cmd_terminate(server->process)) {
        warningLog("Cannot terminate server");
    }
