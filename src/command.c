...
show_adb_err_msg(enum process_result err) {
    switch (err) {
        case PROCESS_ERROR_GENERIC:
            errorLog("Failed to execute adb");
            break;
        case PROCESS_ERROR_MISSING_BINARY:
            errorLog("'adb' command not found (make it accessible from your PATH "
...
process_check_success(process_t proc, const char *name) {
    if (proc == PROCESS_NONE) {
        errorLog("Could not execute \"%s\"", name);
        return false;
    }
    exit_code_t exit_code;
    if (!cmd_simple_wait(proc, &exit_code)) {
        if (exit_code != NO_EXIT_CODE) {
            errorLog("\"%s\" returned with value %" PRIexitcode, name, exit_code);
        } else {
            errorLog("\"%s\" exited unexpectedly", name);
        }
        return false;
    }
    return true;
